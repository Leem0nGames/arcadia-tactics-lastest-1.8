...
const performEnemyAction = (store: GameStore, enemy: Entity & { stats: CombatStatsComponent, position: PositionComponent }, targets: (Entity & { stats: CombatStatsComponent, position: PositionComponent })[], set: (state: Partial<BattleSlice>) => void) => {
    ...
    if (aiBehavior === AIBehavior.SPELLCASTER && enemy.stats.spellSlots.current > 0) {
        ...
        if (dist > 6) {
            ...
            let updatedEntities = store.battleEntities.map((e: Entity & { stats: CombatStatsComponent, position: PositionComponent, visual: VisualComponent }) =>
                e.id === enemy.id
          ...
          if (state.selectedAction === BattleAction.MAGIC && state.selectedSpell) {
               ...
               setTimeout(() => {
                   ...
                   if (spell.type === SpellType.HEAL) {
                       ...
                       if (res) { const updatedEntities = res.battleEntities.map((e: Entity & { stats: CombatStatsComponent, position: PositionComponent, visual: VisualComponent }) => { if (e.id === activeId) return { ...e, stats: { ...e.stats, spellSlots: newSpellSlots } }; return e; }); set({ ...res, battleEntities: updatedEntities }); }
